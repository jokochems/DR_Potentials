...
def walk_data_folder_and_extract_mean(
    ...
    average_temperatures["daily_mean"] = (
        average_temperatures["hourly_mean"].resample("D").mean()
    )
    average_temperatures = average_temperatures.ffill()
...
def read_and_filter_weather_datum(
    ...
    filtered_data = data.loc[data.index.str[:4] == str(year), cols]
    create_datetime_index(filtered_data)
    replace_error_values(filtered_data, error_value)
    return stations_id, filtered_data
