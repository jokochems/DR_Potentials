...
def extract_info(dict_origin, cols_info, drop=True, sample_stats=False,
    ...
    Usage: 
        ...
        - Set sample_stats to True if you wish to include information on the
          sample size. Since this introduces computational expensive
          operations, it is set to False by default.
          The idea is to run it once at the end,
          when a final data analysis set is needed.
        - Set add_sources to True if you wish to include and rearrange sources
          information. Since this introduces computational
          expensive operations, it is set to False by default.
          The idea is to run it once at the end,
          when a final data analysis set is needed.
        - Set modify_categories to True if you wish to use standardized
          categories from the method comparison instead of the given ones.
    ...
    # Create a copy of the columns in order not to manipulate original input
    cols_to_use = cols_info.copy()

    # modify_categories changes the original categories and takes the one of
    # "Prozesskategorie 1" except for cases where replacement mode equals to -1
    if modify_categories:
        cols_to_use.extend(["Prozesskategorie 1", "replacement_mode"])

        if agg_rule == "weighted_average":
            # Remove columns "Prozess" and "year_key" from weights because
            # otherwise, these would be duplicated
            duplicates = ["Prozess", "year_key"]
            weights = [el for el in weights if
                       "Fundstelle" not in el and el not in duplicates]
    ...
    for k, v in dict_origin.items():
        ...
        # Actual modification of categories and aggregate data (if necessary)
        if modify_categories:
            # Replace values for "Prozess" column with those from
            # "Prozesskategorie 1" column if not original names shall be kept
            dict_info[k].loc[dict_info[k]["replacement_mode"] != -1,
                             "Prozess"] = dict_info[k].loc[
                dict_info[k]["replacement_mode"] != -1,
                "Prozesskategorie 1"]
            dict_info[k].loc[
                dict_info[k]["replacement_mode"] == -1, "replacement_mode"] = 0

            # Create a sub DataFrame with duplicate values for
            # "Prozesskategorie 1"
            # Keep all entries in the first place in order to later apply
            # aggregation functions on them and drop from original DataFrame
            sub_df = dict_info[k].loc[
                dict_info[k]["Prozesskategorie 1"].duplicated(keep=False)]
            dict_info[k] = dict_info[k].drop(sub_df.index)

            # Define on which columns of the sub DataFrame to perform
            # (no) aggregation
            # For object columns the 0th value is taken because values
            # are the same (this may not hold for sources information,
            # but is neglected here)
            # For other columns, sum or weighted average apply
            # as aggregation functions
            # After that, group DataFrames by "Prozess" column and
            # combine them again
            no_agg_cols = [col for col in cols
                           if dict_info[k][
                               col].dtype == object or "Fundstelle" in col]
            agg_cols = [col for col in cols
                        if col not in no_agg_cols or col == "Prozess"]

            sub_df_no_agg = sub_df.loc[:, [col for col in cols
                                           if col in no_agg_cols]].groupby(
                "Prozess").nth(0)

            # Weigths used holds the weighting cols used for
            # calculating a weigthed average
            ...
            if not agg_rule == "weighted_average":
                ...
                sub_df_agg = sub_df.loc[:, [col for col in cols
                                            if col in agg_cols]].groupby(
                    "Prozess").sum()

            else:
                weights_used = [el for el in cols
                                if el in weights and el not in ["Prozess",
                                                                "year_key"]]
                # If multiple weight_cols are given, transform into one
                # (using median)
                sub_df["weights"] = sub_df[weights_used].median(axis=1)
                sub_df = sub_df.fillna(sub_df.median(axis=0))

                # Introduce a lambda function for calculating
                # a weighted average
                if not sub_df["weights"].sum() == 0:
                    wm = lambda x: np.average(x, weights=sub_df.loc[
                        x.index, "weights"], axis=0)
                    sub_df_agg = sub_df.loc[:, [col for col in cols
                                                if col in agg_cols]].groupby(
                        "Prozess").aggregate(wm)
                else:
                    sub_df_agg = sub_df.loc[:, [col for col in cols
                                                if col in agg_cols]].groupby(
                        "Prozess").mean()

            sub_df = pd.concat([sub_df_no_agg, sub_df_agg],
            ...
            dict_info[k] = pd.concat([dict_info[k], sub_df], axis=0,
                                     sort=False).drop(
                ["Prozesskategorie 1", "replacement_mode"], axis=1).set_index(
                process_cols)
            # Drop weight columns with information not needed anymore
            # and remove empty columns
            dict_info[k] = dict_info[k].drop(weights_used, axis=1).dropna(
                how="all", axis=1)

        # No modification on categories (take the original ones)
        else:
            # Remove the number of columns by dropping those
            # only containing NaN entries
            dict_info[k] = v[cols].set_index(process_cols).dropna(how="all",
                                                                  axis=1)

        if sample_stats:
            dict_info[k]["info_available"] = \
                dict_info[k].loc[:, [el for el in dict_info[k].columns
                                     if "Fundstelle" not in el]].notnull().any(
                    axis=1).astype(int)
    ...
    for k, v in dict_info.items():
        try:
            df_info = pd.merge(df_info, v, on=process_cols,
                               suffixes=["", "_" + k], how="outer")
        except ValueError as err:
            print(err)
            print(k + " failed!")

    # Add info on the number of publications as well as
    # the number of data values
    # drop redundant info not needed anymore
    if sample_stats:
        df_info["publications_number"] = df_info.filter(
            regex="info_available").sum(axis=1)
        df_info = df_info.drop(
            [col for col in df_info.columns if "info_available" in col],
            axis=1)

        df_info["number_entries"] = df_info.count(axis="columns",
                                                  numeric_only=True) - 1
    ...
    # Drop empty columns or columns containing 3 or less entries
    # (if not sample_stats)
    else:
        if drop:
            cols_to_ignore = process_cols
            df_info = df_info.dropna(how="all",
                                     subset=[el for el in df_info.columns
                                             if not el in cols_to_ignore],
                                     axis=0)

        if drop_datalack:
            df_info = df_info[
                df_info.count(axis="columns", numeric_only=True) >= 3]

    # Concatenate together all sources information and delete
    # source cols not needed anymore
    if add_sources:
...
def create_info_dict_for_years(df_info, years_dict, negate=False,
    ...
    if not isinstance(df_info, list):
        ...
        for year in years_dict.keys():
            dict_info_years[year] = df_info[
                df_info["year_key"] == year].T.drop(
                rows_to_drop, axis="rows").astype(float)
            if "sources" in df_info.columns:
                dict_sources[year] = df_info[df_info["year_key"] == year][
                    "sources"].unique().sum()
            ...
            if negate:
                ...
                if sample_stats:
                    rows_to_ignore.extend(
                        ["publications_number", "number_entries"])
                ...
                dict_info_years[year].loc[rows_to_ignore] = \
                    dict_info_years[year].loc[rows_to_ignore].mul(-1)
        ...
        for el in df_info:
            new_info, new_source = create_info_dict_for_years(
                el, years_dict,
                negate=negate,
...
def create_info_boxplot(dict_info_years, year, title, ylabel, colors,
                        ylim=[0, 3000],
                        use_colors=False, use_limits=True, sample_stats=False,
                        add_sources=False,
    ...
    if sample_stats:
        ...
        to_plot.loc["new_col_names", :] = \
            ...
            ", n: " + to_plot.loc["publications_number"].astype(int).astype(
                str) + \
            ", m: " + to_plot.loc["number_entries"].astype(int).astype(str)

        to_plot.columns = to_plot.loc["new_col_names"]
        to_plot = to_plot.drop(
            ["number_entries", "publications_number", "new_col_names"], axis=0)
    ...
    if not swarmplot:
        ...
        if use_colors:
            _ = sns.boxplot(data=to_plot, ax=ax, width=0.5,
    ...
    if savefig:
        plt.savefig(path_folder + filename + "_boxplot.png", dpi=150,
                    bbox_inches="tight")

    plt.show()


def get_nlargest(dict_info_years, year="SQ", metric="50%", n=5,
                 sample_stats=False):
    """
    Function takes the dictionary containing info on the relevant parameter
    ...
    # If sample_stats is True, drop rows holding publications
    # number resp. number of entries
    if sample_stats:
...
def extract_stats(dict_info_years, years_dict):
    ...
    which are writen into a dictionary indexed by years and then merged
    together to obtain a DataFrame containing overall info.
    ...
    for year in years_dict:
        ...
        if not dict_info_years[year].empty:
            # Obtain statistical info for each process using describe method
            df = dict_info_years[year].describe().T[
                ["min", "max", "mean", "50%"]].dropna(how="all")
            # Melt the DataFrame in order to prepare it for plotting
            # the future development for the respective year (groups)
            dict_stats_years[year] = pd.melt(df.reset_index(),
                                             id_vars="Prozess",
                                             var_name="parameter").set_index(
                ["Prozess", "parameter"])

    df_stats_year = pd.DataFrame(columns=["Prozess", "parameter"])

    for k, v in dict_stats_years.items():
        df_stats_year = pd.merge(df_stats_year, v, on=["Prozess", "parameter"],
                                 suffixes=["", "_" + k], how="outer")
    ...
    df_stats_year = df_stats_year.set_index("year_key", drop=True).drop(
        "index", axis=1)

    return dict_stats_years, df_stats_year


def create_projection_boxplot(df_info, years_dict, process_cols, title, ylabel,
                              colors,
                              ylim=[0, 3000], negate=False, use_colors=False,
                              use_limits=True,
                              sample_stats=False, add_sources=False,
                              swarmplot=False, savefig=False, show_title=True,
                              path_folder="./out/",
    ...
    # Manipulate the DataFrame in order to be able to plot it:
    # select processes of interest
    ...
    # and assign the status quo variable the lowest value to place it at top
    to_plot["sort_col"] = to_plot["year_key"]
    to_plot.loc[to_plot["year_key"] == "SQ", "sort_col"] = 0
    to_plot["sort_col"] = to_plot["sort_col"].astype(int)

    to_plot = to_plot.set_index([to_plot.index, "year_key"]).sort_values(
        by=["sort_col", "Prozess"], ascending=True)
    ...
    if not swarmplot:
        ...
        if use_colors:
            _ = sns.boxplot(data=to_plot, ax=ax, width=0.5,
                            boxprops=dict(alpha=0.2),
                            palette=sns.color_palette(
                                [el for el in colors.loc[sorted(process_cols)][
                                    "Farbe (matplotlib strings)"].values]))
            _ = sns.swarmplot(data=to_plot, ax=ax,
                              palette=sns.color_palette(
                                  [el for el in
                                   colors.loc[sorted(process_cols)][
                                       "Farbe (matplotlib strings)"].values]))
        else:
            _ = sns.boxplot(data=to_plot, ax=ax, width=0.5,
                            boxprops=dict(alpha=0.2))
    ...
    if savefig:
        plt.savefig(path_folder + filename + "_projection_box.png", dpi=150,
                    bbox_inches="tight")
...
def save_future_sources(df_info, process_cols, path_folder, filename,
    ...
    if add_sources:
        dict_sources["sources"] = \
            df_info.loc[[el for el in process_cols if el in df_info.index], :][
                "sources"].unique().sum()
